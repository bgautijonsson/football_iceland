...
posterior_goals |>
  write_csv(
    here("results", "female", "posterior_goals.csv")
  )
...
plot <- predictions |>
  ...
  labs(
    ...
    colour = NULL,
    title = paste0(
      "Vikuleg fótboltaspá Metils fyrir Bestu deild (BD) kvenna og suma Mjólkurbikarsleiki (MB)"
    ),
    subtitle = str_c(
...
plot_dat |>
  ...
  labs(
    title = "Hvaða lið munu komast í efri hluta umspils Bestu deildar kvenna",
    subtitle = str_c(
...
ggsave(
  filename = here("results", "female", "figures", "umspil_top.png"),
  width = 8,
  height = 0.5 * 8,
  scale = 1.6
)
...
plot_dat |>
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Líkindadreifing yfir lokasæti í Bestu deild kvenna fyrir umspil haustsins"
  )

ggsave(
  filename = here("results", "female", "figures", "deild_top.png"),
  width = 8,
  height = 0.5 * 8,
  scale = 1.2
)
...
plot_dat |>
  ...
  labs(
    ...
    y = NULL,
    colour = NULL,
    title = "Styrkur félagsliða í Bestu deild kvenna",
    subtitle = "Metið með fótboltalíkani Metils"
  )

ggsave(
  filename = here("results", "female", "figures", "styrkur.png"),
  width = 8,
  height = 0.4 * 8,
  scale = 1.1
)
...
results$draws("home_advantage_tot") |>
  ...
  labs(
    ...
    y = NULL,
    colour = NULL,
    title = "Heimavallaráhrif í Bestu deild kvenna",
