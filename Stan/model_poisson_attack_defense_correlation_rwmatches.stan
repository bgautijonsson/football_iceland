...
functions {
  ...
  real poisson_2d_log_lpmf(
    ...
    // If either count is negative, this probability is 0 on the standard domain
    if (x1 < 0 || x2 < 0)
      return negative_infinity();

    // Precompute the exponential of the log lambdas
    ...
    // Build up the individual log-terms and then use log_sum_exp
    vector[K + 1] log_terms;

    for (k in 0:K) {
      // term in log scale:
      //   k*log_lambda3 + (x1-k)*log_lambda1 + (x2-k)*log_lambda2
    ...
    // Combine the exponential constant part with the log-sum-exp of the summation.
    // The "constant" part is  - (lambda1 + lambda2 + lambda3)
    return
...
model {
  ...
  mean_sigma_off ~ normal(log(0.2), 1);

  z_sigma_def ~ std_normal();
  scale_sigma_def ~ exponential(1);
  mean_sigma_def ~ normal(log(0.2), 1);
  ...
  for (n in 1:N) {
    ...
    real logit_rho = alpha_mu3 + team_alpha_mu3[team1[n]] + team_alpha_mu3[team2[n]];
...
generated quantities {
  ...
  for (n in 1:N_pred) {
    ...
    real logit_rho = alpha_mu3 + team_alpha_mu3[team1_pred[n]] + team_alpha_mu3[team2_pred[n]];
    real mu3 = log_inv_logit(logit_rho) + 0.5 * (mu[1] + mu[2]);
